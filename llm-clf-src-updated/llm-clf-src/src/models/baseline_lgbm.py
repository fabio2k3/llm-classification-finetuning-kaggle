...
def train_baseline():
    ...
    for fold in range(n_folds):
        ...
        fold_logloss = log_loss(y_val, oof_preds[val_idx.values], labels=[0, 1, 2])
        print(f"Fold {fold} log loss: {fold_logloss:.5f}")

    overall_logloss = log_loss(train_feat["target"], oof_preds, labels=[0, 1, 2])
    print(f"\n{'='*50}")
    ...
    # Envuelto en try/except: esto es informativo, nunca debe tumbar el submission
    # ya generado arriba si algo falla al guardar el gráfico.
    try:
        print_feature_importance(model, feature_cols, output_dir=cfg["paths"]["outputs_dir"])
    except Exception as e:
        print(f"\n[aviso] No se pudo guardar el gráfico de feature importance: {e}")
        print("[aviso] Esto no afecta el submission.csv ya generado, continuando.")
